    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-6 text-slate-900 dark:text-slate-200'>
      ...
      >
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-xl font-semibold'>Ai Background Remove</h1>
        </div>
        ...
        <p className='font-xs text-gray-500 font-light mt-1'>
          Supports JPG, PNG, and all other image formats.
        </p>

        <button
          disabled={loading}
          type='submit'
          className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#f6ab41] to-[#04ff50] px-4 py-2 rounded-lg hover:bg-[#4338CA] transition-colors'
        >
          {loading ? (
            <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent border-white animate-spin'></span>
          ) : (
            <Eraser className='w-5 h-5 inline mr-2' />
          )}
          Remove Background
        </button>
      </form>
      ...
      <div className='w-full flex-1 min-w-lg bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 shadow-lg min-h-96 overflow-y-auto'>
        <div className='text-sm text-gray-500 gap-3'>
          <Eraser className='text-[#ff4938] w-4 h-5' />
          <h1 className='text-xl font-semibold'>Background removed</h1>
        </div>
        {!content ? (
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-gray-400 gap-5 flex flex-col items-center'>
              ...
              <p className='text-sm'>
                Once your file is uploaded, just click &quot;Remove background&quot;.
              </p>
            </div>
          </div>
        ) : (
